...
import { Loader2, CarFront, AlertTriangle, Archive, Building2, BookMarked, ShieldAlert } from "lucide-react";
...
const CustomTooltip = ({
  ...
  if (active && payload && payload.length) {
    ...
    return <div className="bg-white/95 dark:bg-gray-800/95 p-4 rounded-lg shadow-lg border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
...
const StatusGroup = ({
  ...
  return <div className={cn("space-y-2", className)}>
      ...
      <div className="space-y-1">
        {items.map(item => <div key={item.status} className="flex items-center justify-between group cursor-pointer p-1 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full transition-transform duration-200 group-hover:scale-110" style={{
...
const CustomLegend = ({
  ...
  return <div className="grid grid-cols-3 gap-4 mt-6">
      <StatusGroup title="Operational" items={groupedData.operational} className="border-r border-gray-200 dark:border-gray-700 pr-4" />
      <StatusGroup title="Needs Attention" items={groupedData.attention} className="border-r border-gray-200 dark:border-gray-700 pr-4" />
      <StatusGroup title="Critical" items={groupedData.critical} />
    </div>;
};
export const VehicleStatusChart = ({
  ...
  return <Card className={cn("bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-gray-200/50 dark:border-gray-700/50 hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-300", className)}>
      <CardHeader>
      ...
      <CardContent>
        <div className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={vehicles} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={120} innerRadius={85} paddingAngle={2} startAngle={90} endAngle={-270}>
                {vehicles.map(entry => <Cell key={`cell-${entry.status}`} fill={entry.color} className="transition-all duration-300 hover:opacity-80 hover:scale-105" style={{
