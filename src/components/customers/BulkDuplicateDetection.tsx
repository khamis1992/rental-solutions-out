...
export function BulkDuplicateDetection() {
  ...
  const mergeSelectedClusters = async () => {
    try {
      setIsMerging(true);
      const selectedClusterIds = Object.entries(selectedClusters)
        .filter(([_, value]) => value.selected && value.primaryId)
      ...
      for (const clusterId of selectedClusterIds) {
        ...
        const duplicateIds = cluster.customers
          .map(c => c.id)
          .filter(id => id !== primaryId);

        // Update all references to duplicate IDs to point to the primary ID
        const { error: updateError } = await supabase.rpc('merge_customer_records', {
                ...
                <CollapsibleTrigger className="w-full">
                  <Card className="flex items-center justify-between p-4 hover:bg-accent">
                    <div className="flex items-center gap-4">
                      <Checkbox
                        checked={selectedClusters[index]?.selected}
                  ...
                  <Card className="mt-2 p-4">
                    <div className="space-y-2">
                      {cluster.customers.map((customer) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            {selectedClusters[index]?.selected && (
                              <Checkbox
                                checked={selectedClusters[index]?.primaryId === customer.id}
                                onCheckedChange={() => handlePrimarySelection(index.toString(), customer.id)}
                              />
                            )}
                            <div>
