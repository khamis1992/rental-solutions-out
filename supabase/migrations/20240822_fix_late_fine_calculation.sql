...
-- Update the get_pending_payments_report function to fix the column ambiguity error
...
DECLARE
  ...
  default_daily_late_fee numeric := 120; -- Default late fee per day in QAR
...
BEGIN
  ...
  WITH active_agreements AS (
    ...
    SELECT 
      ...
      p.phone_number,
      v.license_plate,
      COALESCE(l.daily_late_fee, default_daily_late_fee) AS daily_late_fee
    FROM leases l
